...
class users extends database
{
    ...
    /**
     * Méthode construct qui va permettre de construire notre class 
     */
    ...
    public function getUsersInfos() {
        $query = 'SELECT `d27PG_users`.`id`,`d27PG_users`.`lastname`, `d27PG_users`.`firstname`, `d27PG_users`.`email`, `d27PG_users`.`password`, `d27PG_users`.`firm`, `d27PG_users`.`address`, `d27PG_users`.`phoneNumber`, `d27PJ_city`.`postalCode`, `d27PG_users`.`idUsersType`, `d27PJ_city`.`cityName` '
            . ' FROM `d27PG_users`'
        ...
        $result->execute();  //On vérifie que la requête s'est bien exécutée
        if (is_object($result)) { //On vérifie que l'on a bien trouvé un utilisateur
            $resultUsers = $PDOResult->fetch(PDO::FETCH_OBJ);
        }
    ...
    {
        $remove = $this->db->prepare('DELETE FROM `d27PG_users` WHERE `id`=:idRemove');
