...
class projects extends database {
    ...
    public function projectsInsertion() {
        ...
        // Etant donné que les données vont être entrées par l'utilisateur on fait un prepare
    ...
    public function getProjectInfos() {
        // et on finit par un execute
        $PDOResult = $this->db->prepare('SELECT `d27PD_projects`.`id`,`d27PD_projects`.`area`,`d27PD_projects`.`address`, `d27PD_projects`.`startBudget`, `d27PD_projects`.`endBudget`, `d27PD_projects`.`moreInfos`, `d27PA_rooms`.`room`, `d27PF_propertyTypes`.`type`, `d27PG_users`.`firstname`, `d27PJ_city`.`cityName` '
                . 'FROM `d27PD_projects` '
    ...
    public function getProjectInfosOnce() {
        // et on finit par un execute
        $PDOResult = $this->db->prepare('SELECT `d27PD_projects`.`id`, `d27PD_projects`.`name`,`d27PD_projects`.`area`,`d27PD_projects`.`address`, `d27PD_projects`.`startBudget`, `d27PD_projects`.`endBudget`, `d27PD_projects`.`moreInfos`, `d27PA_rooms`.`room`, `d27PF_propertyTypes`.`type`, `d27PG_users`.`firstname`, `d27PJ_city`.`cityName` '
                . 'FROM `d27PD_projects` '
                . 'INNER JOIN `d27PA_rooms` '
                . 'ON `d27PD_projects`.`idRooms` = `d27PA_rooms`.`id` '
                . 'INNER JOIN `d27PG_users` '
